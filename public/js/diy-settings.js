    ...
    tabConfigs: {
      visual: {
        ...
        fields: [
          ...
          { key: 'enableParticleCover', label: '启用粒子封面', type: 'toggle', help: '3D粒子化专辑封面，GPU密集。关闭后显示静态缩略图' },
          { key: 'particleResolution', label: '粒子分辨率', type: 'range', min: 60, max: 200, step: 1, help: '粒子网格密度。越高越细腻但GPU消耗越大。默认160' },
          { key: 'particleScatterStrength', label: '散落强度', type: 'range', min: 0, max: 1, step: 0.05, help: '切歌时粒子散开的距离和力度' },
          ...
          { key: 'enableFluidBg', label: '启用流体背景', type: 'toggle', help: '全屏动态水波纹背景，GPU占用较低' },
          { key: 'bgIntensity', label: '背景强度', type: 'range', min: 0, max: 1, step: 0.05, help: '水波纹的明显程度。0=几乎静止，1=强烈波动' },
          { key: 'bgSpeed', label: '背景速度', type: 'range', min: 0.1, max: 3, step: 0.1, help: '水波纹流动速度' },
          { key: 'bgNoiseScale', label: '噪声质量', type: 'range', min: 0.25, max: 1.0, step: 0.05, help: '背景噪声分辨率。越低GPU越省但画面更模糊。推荐0.5平衡' },
          { type: 'section', label: '━━ 歌单列表 ━━' },
          { key: 'playlistTextColor', label: '列表文字颜色', type: 'color', help: '左侧歌单歌曲列表的文字颜色' },
          { type: 'section', label: '━━ 歌词 ━━' },
          { key: 'lyricsVisibleLines', label: '显示行数', type: 'range', min: 0, max: 40, step: 1, help: '右侧歌词仓最大显示行数。0=全部' },
          { key: 'lyricsFontSize', label: '歌词字号', type: 'range', min: 10, max: 24, step: 1, help: '右侧歌词仓的字体大小' },
          { key: 'lyricsColor', label: '歌词颜色', type: 'color', help: '歌词文字颜色，点击色块调色' },
          { key: 'lyricsHighlightColor', label: '高亮颜色', type: 'color', help: '当前播放句高亮色' },
          { key: 'lyricsFadeStrength', label: '歌词淡化', type: 'range', min: 0, max: 1, step: 0.05, help: '歌词上下边缘的淡化程度' },
          { key: 'lyricsEffect', label: '动态效果', type: 'select', options: { fade: '渐隐渐显', glow: '发光脉冲', typewriter: '逐字显现', none: '无效果' }, help: '歌词行切换时的过渡动画' },
          { key: 'inlineLyricColor', label: '行内歌词颜色', type: 'color', help: '中心核心区单行歌词颜色' },
          { key: 'inlineLyricFontSize', label: '行内歌词字号', type: 'range', min: 12, max: 22, step: 1, help: '中心核心区单行歌词字号' },
          ...
          { key: 'wallpaperOpacity', label: '背景透明度', type: 'range', min: 0, max: 1, step: 0.05, help: '自定义背景的显示透明度。0=不显示背景' },
          { key: 'wallpaperRippleSpeed', label: '水波纹速度', type: 'range', min: 0.2, max: 2, step: 0.1, help: '背景上叠加的水波纹流动速度' },
        ],
      },
      system: {
        title: '⚙️ 系统',
